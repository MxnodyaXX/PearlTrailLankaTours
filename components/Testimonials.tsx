        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {reviews.map((r) => (
            <div
              key={r.name}
              data-glow
              className="bg-white/[.08] border border-white/[.13] backdrop-blur-xl rounded-[24px] p-6 text-white
                hover:bg-white/[.13] transition-colors duration-300"
            >
              <div className="flex gap-0.5 text-gold text-sm mb-4">
                {"★★★★★".split("").map((s, i) => <span key={i}>{s}</span>)}
              </div>
              <p className="text-white/80 text-[15px] leading-relaxed italic mb-5">"{r.text}"</p>
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-full bg-gold flex items-center justify-center text-[#0f172a] font-black text-sm shrink-0">
                  {r.init}
                </div>
                <div>
